...
public class ChoiceService {
    ...
    public List<ChoiceEntity> selectChoiceList(Integer themeId){
        return choiceRepo.selectChoiceList(themeId)
            .stream()
            .map(record -> new ChoiceEntity(record.getId(), record.getChoice(), record.getThemeId()))
    ...
    public ChoiceEntity insertChoice(Integer themeId, String choice){
        choiceRepo.insertChoice(choice, themeId);
        return null;
    }

    public void deleteChoiceList(Integer themeId){
        choiceRepo.deleteChoiceList(themeId);
    }

    public void deleteChoice(Integer id){
        choiceRepo.deleteChoice(id);
    }
    ...
    public List<ChoiceEntity> updateChoiceList(Integer themeId, List<ChoiceDTO> dtoList) {
        ...
        List<ChoiceEntity> entityList = dtoList.stream().map(
            dto -> new ChoiceEntity(dto.getId(), dto.getChoice(), dto.getThemeId())
        ).collect(Collectors.toList());
        ...
        List<ChoiceEntity> entityList
            ...
            for (ChoiceEntity entity : entityList){
                ...
                if (entity.getId() == 0){
                    ...
                    choiceRepo.updateChoice(entity);
                }
            }

            // prevEntityListをloopしてentityList内にidがあるか判定し削除処理
            for (ChoiceEntity entity: prevEntityList){
                if(!entityList.stream().anyMatch(item -> item.getId().equals(entity.getId()))){
                    deleteChoice(entity.getId());
                }
            }
